...
public class ThreadPoolsDemo2 {
    public static void main(String[] args) {
        ...
        // c.把线程任务放入到线程池中去执行
        pools.submit(target);  // 第1次提交任务，此时会创建一个新线程，自动启动线程执行！
        pools.submit(target);  // 第2次提交任务，此时会创建一个新线程，自动启动线程执行！
        pools.submit(target);  // 第3次提交任务，此时会创建一个新线程，自动启动线程执行！
        pools.submit(target);  // 第4次提交任务，不会再创建新线程，会复用之前的线程来处理这个任务

        pools.shutdown();  // 等待任务执行完毕以后才会关闭线程池
        // pools.shutdownNow();  // 立即关闭线程池，无论任务是否执行完毕！
